			...
			<div class="score">
				<div class="views">
					<p class="vote_average">
						${film.vote_average}
						<img src="images/star.svg" style="width: 1em; margin: 2px;">
					</p>
				</div>
				<div class="mark"><p class="vote_count">${film.vote_count}</p></div>
			</div>
		</div>
	`
	return card 
...
function editFilms(page, API) {
	getMovie(API).then(data => {
		...
		for (let i = 0; i < films.length; i++) {
			...
			card[i].querySelector('.vote_average').innerHTML = film.vote_average
			card[i].querySelector('.vote_count').innerHTML = film.vote_count
		}
	})
}
...
function howChange(change, curPage) {
	...
	const API = `${BASE_URL}/discover/movie?page=${activePage}&${API_KEY}`
